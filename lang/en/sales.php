<?php

declare(strict_types=1);

return [

    'navigation_group' => 'Sales',
    'products_group' => 'Products and Costs',

    'taxes' => [
        'label' => 'Tax',
        'plural_label' => 'Taxes',
        'nav_label' => 'Taxes',
        'columns' => [
            'number' => 'No.',
            'name' => 'Name',
            'code' => 'Code',
            'rate' => 'Rate',
            'account' => 'Account',
            'active' => 'Active',
            'default' => 'Default',
        ],
        'sections' => [
            'details' => 'Tax details',
            'posting' => 'Posting',
        ],
        'fields' => [
            'name' => 'Name',
            'name_en' => 'Name in English',
            'category' => 'Code',
            'rate' => 'Rate',
            'account' => 'Account',
            'is_active' => 'Active',
            'is_default' => 'Default tax',
        ],
        'hints' => [
            'category' => 'The ZATCA tax category code carried on an electronic invoice.',
            'rate' => 'A percentage. Zero-rated and exempt are always zero.',
            'account' => 'Where the tax posts when a document is approved.',
            'is_default' => 'Applied to document lines that name no tax of their own.',
        ],
        'errors' => [
            'rate_on_zero_category' => 'Zero-rated and exempt taxes cannot carry a rate above zero.',
            'system_delete' => 'A system tax cannot be deleted. Deactivate it instead.',
        ],
    ],

    'tax_category' => [
        'S' => 'Standard rated',
        'Z' => 'Zero rated',
        'E' => 'Exempt',
    ],

    'invoice_subtype' => [
        '01' => 'Tax invoice',
        '02' => 'Simplified tax invoice',
    ],

    'document_status' => [
        'draft' => 'Draft',
        'approved' => 'Approved',
        'void' => 'Void',
    ],

    'quotation_status' => [
        'draft' => 'Draft',
        'approved' => 'Approved',
        'invoiced' => 'Invoiced',
        'cancelled' => 'Cancelled',
        'expired' => 'Expired',
    ],

    'quotations' => [
        'label' => 'Quotation',
        'plural_label' => 'Quotations',
        'nav_label' => 'Quotations',
        'from_quotation' => 'From quotation :reference',
        'columns' => [
            'reference' => 'Quotation No.',
            'contact' => 'Customer',
            'issue_date' => 'Issue Date',
            'expiry_date' => 'Expiry Date',
            'status' => 'Status',
            'net' => 'Total Before Tax',
            'tax' => 'Tax Amount',
            'total' => 'Total',
        ],
        'filters' => [
            'expired' => 'Expired',
        ],
        'sections' => [
            'details' => 'Quotation Details',
            'items' => 'Items',
            'notes' => 'Notes & Terms',
        ],
        'fields' => [
            'reference' => 'Quotation No.',
            'description' => 'Quotation Description',
            'contact' => 'Customer Name',
            'issue_date' => 'Issue Date',
            'expiry_date' => 'Expiry Date',
            'terms_and_conditions' => 'Terms & Conditions',
            'notes' => 'Notes',
        ],
        'actions' => [
            'save_draft' => 'Save as Draft',
            'approve' => 'Save & Approve',
            'approve_confirm' => 'Approving fixes the quotation. It affects neither the accounts nor the reports - no entry is posted. Posting happens only when the invoice created by conversion is approved.',
            'approved' => 'The quotation has been approved.',
            'convert' => 'Convert to Invoice',
            'convert_confirm' => 'A draft invoice will be created from this quotation and the quotation becomes Invoiced. Taxes are computed at the invoice date rates.',
            'convert_expired_warning' => 'This quotation expired on :date. You may proceed; the invoice is created as a reviewable draft.',
            'converted' => 'Draft invoice :reference has been created.',
            'cancel' => 'Cancel',
            'cancel_confirm' => 'The quotation will be cancelled. Cancellation cannot be undone.',
            'cancelled' => 'The quotation has been cancelled.',
        ],
        'hints' => [
            'expiry_date' => 'How long the offer stands - after this date the quotation is considered expired.',
        ],
        'errors' => [
            'no_items' => 'A quotation with no items cannot be approved.',
            'already_approved' => 'Quotation :reference is already approved.',
            'not_draft' => 'A quotation cannot be edited after approval.',
            'inactive_contact' => 'Customer :contact is inactive.',
            'expiry_before_issue' => 'The expiry date cannot precede the issue date.',
            'totals_do_not_reconcile' => 'The totals of quotation :reference do not match its items.',
            'not_approved' => 'Quotation :reference cannot be converted - only approved quotations convert.',
            'already_invoiced' => 'Quotation :reference has already been converted to invoice :invoice.',
            'tax_no_longer_available' => 'The tax ":tax" used on this quotation is no longer available. Update the quotation items before converting.',
            'cannot_cancel' => 'Quotation :reference cannot be cancelled in its current state.',
        ],
    ],

    'invoices' => [
        'label' => 'Sales Invoice',
        'plural_label' => 'Sales Invoices',
        'nav_label' => 'Sales Invoices',
        'create_title' => 'Create sales invoice',
        'narration' => 'Sales invoice :reference — :customer',
        'columns' => [
            'reference' => 'Reference',
            'contact' => 'Customer',
            'issue_date' => 'Issue date',
            'due_date' => 'Due date',
            'status' => 'Status',
            'payment' => 'Payment',
            'net' => 'Net',
            'tax' => 'Tax',
            'total' => 'Total',
        ],
        'sections' => [
            'details' => 'Invoice details',
            'items' => 'Items',
            'totals' => 'Totals',
            'notes' => 'Notes and terms',
        ],
        'fields' => [
            'reference' => 'Reference',
            'description' => 'Description',
            'contact' => 'Customer',
            'issue_date' => 'Issue date',
            'payment_term' => 'Payment terms',
            'due_date' => 'Due date',
            'supply_date' => 'Supply date',
            'subtype' => 'Invoice type',
            'terms_and_conditions' => 'Terms and conditions',
            'notes' => 'Notes',
        ],
        'items' => [
            'product' => 'Product',
            'description' => 'Description',
            'quantity' => 'Quantity',
            'unit_price' => 'Unit price',
            'is_inclusive' => 'Inclusive?',
            'discount' => 'Discount',
            'net' => 'Net',
            'tax_rate' => 'Tax %',
            'tax_amount' => 'Tax amount',
            'line_total' => 'Amount',
            'add' => 'Add item',
            'discount_kind' => 'Discount type',
        ],
        'discount_type' => [
            'percentage' => 'Percent %',
            'amount' => 'Amount',
        ],
        'totals' => [
            'net' => 'Net total',
            'discount' => 'Total discount',
            'tax' => 'Total tax',
            'grand' => 'Total',
            'summary' => 'Net :net · Tax :tax · Total :total',
        ],
        'actions' => [
            'save_draft' => 'Save as draft',
            'approve' => 'Save and approve',
            'approve_confirm' => 'Once approved the invoice posts to the accounts and appears in reports. It can then only be changed by a credit note.',
            'approved' => 'Invoice approved and posted.',
            'void' => 'Void invoice',
        ],
        'hints' => [
            'supply_date' => 'When the goods were supplied or the service performed. ZATCA requires it separately from the issue date.',
            'is_inclusive' => 'Is the unit price inclusive of VAT?',
            'subtype' => 'Standard for VAT-registered customers, simplified for consumers.',
        ],
        'errors' => [
            'no_items' => 'An invoice with no items cannot be approved.',
            'already_approved' => 'Invoice :reference is already approved. Correction is by credit note.',
            'not_draft' => 'An invoice cannot be edited once approved.',
            'inactive_contact' => 'Customer :contact is inactive and cannot be invoiced.',
            'due_before_issue' => 'The due date cannot fall before the issue date.',
            'simplified_for_registered' => 'Customer :contact is VAT-registered and needs a standard tax invoice, not a simplified one — a simplified invoice gives them nothing to recover input VAT with.',
            'discount_exceeds_line' => 'The discount is larger than the line it applies to.',
            'negative_discount' => 'A discount cannot be negative.',
            'totals_do_not_reconcile' => 'The totals on invoice :reference do not agree with its lines.',
        ],
    ],

    'credit_note_reason' => [
        'cancellation' => 'Supply cancelled, wholly or partly',
        'supply_change' => 'Material change to the supply affecting the tax',
        'price_adjustment' => 'Adjustment to the agreed value of the supply',
        'goods_return' => 'Return of goods or services',
    ],

    'credit_notes' => [
        'label' => 'Credit Note',
        'plural_label' => 'Credit Notes',
        'nav_label' => 'Credit Notes',
        'narration' => 'Credit note :reference against invoice :invoice',
        'actions' => [
            'approve_confirm' => 'Once approved the credit note posts to the accounts and reduces what the customer owes. It cannot then be edited.',
            'approved' => 'Credit note approved and posted.',
        ],
        'fields' => [
            'reference' => 'Reference',
            'contact' => 'Customer',
            'parent' => 'Original sales invoice',
            'original_invoice_number' => 'Original invoice reference',
            'original_invoice_date' => 'Original invoice date',
            'issue_date' => 'Date',
            'due_date' => 'Due date',
            'event_date' => 'Event date',
            'reason_code' => 'Reason for issue',
            'reason_text' => 'Reason detail',
        ],
        'hints' => [
            'original_invoice_number' => 'Always required, even when the original invoice was raised outside this system.',
            'event_date' => 'When the triggering event occurred. The fifteen-day window runs from the end of that month.',
            'reason_text' => 'Printed on the note, and required by ZATCA.',
        ],
        'errors' => [
            'already_approved' => 'Credit note :reference is already approved.',
            'not_draft' => 'A credit note cannot be edited once approved.',
            'no_items' => 'A credit note with no items cannot be approved.',
            'nothing_to_credit' => 'Credit note :reference comes to zero, so there is nothing to credit.',
            'inactive_contact' => 'Customer :contact is inactive.',
            'totals_do_not_reconcile' => 'The totals on credit note :reference do not agree with its lines.',
            'parent_not_approved' => 'Invoice :reference is not approved, so it cannot be credited.',
            'customer_mismatch' => 'This credit note names a different customer from invoice :invoice.',
            'dated_before_invoice' => 'The credit note is dated before invoice :invoice.',
            'exceeds_invoice' => 'This exceeds what is left of invoice :invoice — :remaining remains.',
        ],
    ],

    'payment_status' => [
        'unpaid' => 'Unpaid',
        'partially_paid' => 'Partially paid',
        'paid' => 'Paid',
    ],

    'receipts' => [
        'label' => 'Receipt',
        'plural_label' => 'Customer Receipts',
        'nav_label' => 'Customer Receipts',
        'narration' => 'Receipt :reference — :customer',
        'allocation_narration' => 'Allocation from receipt :reference to invoice :invoice',
        'unallocation_narration' => 'Allocation released from receipt :reference',
        'columns' => [
            'reference' => 'Reference',
            'contact' => 'Customer',
            'account' => 'Account',
            'date' => 'Date',
            'amount' => 'Amount',
            'allocated' => 'Used',
            'unallocated' => 'Unused',
            'status' => 'Status',
        ],
        'fields' => [
            'reference' => 'Reference',
            'contact' => 'Customer',
            'deposit_account' => 'Account',
            'receipt_date' => 'Date',
            'amount' => 'Amount',
            'payment_method' => 'Payment method',
            'payment_reference' => 'Bank / cheque reference',
            'description' => 'Description',
            'notes' => 'Notes',
        ],
        'allocations' => [
            'title' => 'Allocate against invoices',
            'invoice' => 'Invoice',
            'outstanding' => 'Outstanding',
            'amount' => 'Allocated amount',
            'add' => 'Add invoice',
            'summary' => 'Allocated :allocated · Unallocated :unallocated',
            'allocate' => 'Allocate to invoice',
            'unallocate' => 'Release allocation',
            'allocated' => 'Allocated.',
            'unallocated_done' => 'Allocation released.',
            'date' => 'Allocation date',
        ],
        'hints' => [
            'deposit_account' => 'A cash or bank account with the payment flag enabled.',
            'amount' => 'The total money received. Whatever is not allocated to an invoice is held as a customer advance.',
        ],
        'actions' => [
            'approve_confirm' => 'Once approved the receipt posts to the accounts, any unallocated remainder is held as a customer advance, and it cannot be edited.',
            'approved' => 'Receipt approved and posted.',
        ],
        'errors' => [
            'already_approved' => 'Receipt :reference is already approved.',
            'not_draft' => 'A receipt cannot be changed once approved.',
            'nothing_received' => 'Receipt :reference carries no amount.',
            'inactive_contact' => 'Customer :contact is inactive.',
            'not_a_customer' => ':contact is not a customer.',
            'deposit_account_invalid' => 'Account :account is not enabled for payments. Enable the flag on it in the chart of accounts, or choose a cash or bank account.',
            'allocation_not_positive' => 'An allocation must be greater than zero.',
            'allocations_exceed_amount' => 'The allocations exceed the amount of receipt :reference.',
            'invoice_not_found' => 'The invoice does not exist.',
            'invoice_not_approved' => 'Invoice :invoice is not approved, so nothing can be allocated to it.',
            'customer_mismatch' => 'Invoice :invoice belongs to a different customer.',
            'currency_mismatch' => 'Invoice :invoice is in a different currency from the receipt.',
            'dated_before_invoice' => 'The receipt predates invoice :invoice. Money received before an invoice is approved unallocated and applied later.',
            'exceeds_outstanding' => 'The allocation exceeds what is outstanding on invoice :invoice — :outstanding remains.',
            'invoice_already_allocated' => 'This receipt already has an allocation on invoice :invoice. Release it, then allocate again.',
            'exceeds_unallocated' => 'The amount exceeds the unallocated part of receipt :reference — :unallocated remains.',
        ],
    ],

    'product_type' => [
        'product' => 'Product',
        'bundle' => 'Bundle',
        'raw_material' => 'Raw material',
        'service' => 'Service',
        'expense' => 'Expense',
    ],

    'products' => [
        'label' => 'Product',
        'plural_label' => 'Products and Costs',
        'nav_label' => 'Products and Costs',
        'columns' => [
            'sku' => 'SKU',
            'name' => 'Name',
            'type' => 'Type',
            'category' => 'Category',
            'unit' => 'Unit',
            'tax' => 'Tax',
            'selling_price' => 'Selling price',
            'buying_price' => 'Buying price',
            'expense_account' => 'Expense Account',
            'active' => 'Active',
        ],
        'sections' => [
            'details' => 'Product details',
            'pricing' => 'Pricing',
        ],
        'fields' => [
            'type' => 'Product type',
            'name' => 'Name in Arabic',
            'name_en' => 'Name in English',
            'sku' => 'SKU',
            'barcode' => 'Barcode',
            'category' => 'Category',
            'unit_type' => 'Unit',
            'tax' => 'Tax %',
            'description' => 'Description',
            'terms_and_conditions' => 'Terms and conditions',
            'is_sold' => 'Sold',
            'is_purchased' => 'Purchased',
            'selling_price' => 'Selling price',
            'buying_price' => 'Buying price',
            'is_active' => 'Active',
        ],
        'hints' => [
            'expense_account' => 'The account purchases of this product post to. Defaults to Cost of Goods Sold.',
            'sku' => 'Generated automatically if left blank.',
            'name_en' => 'Required because a tax invoice may be issued in either language.',
            'selling_price' => 'A default documents copy; changing it later does not alter invoices already raised.',
        ],
    ],

    'product_categories' => [
        'label' => 'Category',
        'plural_label' => 'Categories',
        'nav_label' => 'Categories',
        'columns' => [
            'name' => 'Name',
            'parent' => 'Parent category',
            'description' => 'Description',
        ],
        'fields' => [
            'name' => 'Category name',
            'description' => 'Description',
            'parent' => 'Parent category',
        ],
    ],

    'contact_type' => [
        'customer' => 'Customer',
        'supplier' => 'Supplier',
    ],

    'contact_status' => [
        'active' => 'Active',
        'inactive' => 'Inactive',
    ],

    'contacts' => [
        'customer_label' => 'Customer',
        'customers_label' => 'Customers',
        'nav_label' => 'Customers',
        'columns' => [
            'code' => 'Reference',
            'name' => 'Customer name',
            'organization' => 'Organisation',
            'phone' => 'Phone',
            'email' => 'Email',
            'tax_number' => 'VAT number',
            'status' => 'Status',
        ],
        'sections' => [
            'details' => 'Customer details',
            'billing_address' => 'Billing address',
            'shipping_address' => 'Shipping address',
            'bank' => 'Bank account',
        ],
        'fields' => [
            'code' => 'Reference',
            'contact_name' => 'Customer name',
            'organization_name' => 'Organisation name',
            'primary_contact_number' => 'Primary phone',
            'secondary_contact_number' => 'Secondary phone',
            'primary_email' => 'Primary email',
            'secondary_email' => 'Secondary email',
            'website' => 'Website',
            'tax_number' => 'VAT number',
            'status' => 'Status',
            'currency' => 'Currency',
            'is_pos' => 'Point of sale customer',
            'is_government_entity' => 'Government entity',
            'address' => 'Address',
            'city' => 'City',
            'state' => 'Region',
            'zip' => 'Postal code',
            'building_number' => 'Building number',
            'country' => 'Country',
            'copy_billing' => 'Copy billing address',
            'bank_name' => 'Bank name',
            'bank_account_name' => 'Account holder',
            'bank_country' => 'Country',
            'bank_currency' => 'Currency',
            'bank_iban' => 'IBAN',
            'bank_account_number' => 'Account number',
            'bank_swift_code' => 'SWIFT code',
            'bank_address' => 'Bank address',
        ],
        'hints' => [
            'code' => 'Generated automatically if left blank.',
            'tax_number' => "The VAT registration number, printed on the customer's tax invoices.",
            'building_number' => 'Part of the Saudi national address a tax invoice must carry.',
            'is_government_entity' => 'Cannot be cleared once set — sales to government bodies are reported differently.',
        ],
        'errors' => [
            'government_entity_locked' => 'The government entity flag on :contact cannot be cleared once set.',
        ],
    ],

];
