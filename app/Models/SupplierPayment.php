...
/**
 * A supplier payment voucher — سند صرف.
 *
 * Money paid out of a payment account and allocated across the supplier's
 * bills. Whatever is not allocated is held as an advance to the supplier —
 * an ASSET, the mirror of the customer side: it is our money until an
 * allocation turns it into settlement.
 *
 * No stored balance: how much of the voucher is used derives from its
 * allocation rows, exactly as a bill's payment state derives from the
 * vouchers against it.
 *
 ...
 */
#[Fillable([
    'company_id', 'reference', 'status', 'contact_id', 'payment_account_id',
...
class SupplierPayment extends Model implements AuditableContract
{
    ...
    {
        return $this->hasMany(SupplierPaymentAllocation::class)->orderBy('line_number');
    }
    ...
    /**
     * The sum of this voucher's allocation rows.
     */
    ...
    /**
     * What is not yet applied to any bill — the advance.
     */
