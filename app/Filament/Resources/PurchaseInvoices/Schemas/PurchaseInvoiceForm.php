 ...
 *
 * The sales invoice form with the seller's fields removed and the buyer's
 * added. Gone: the subtype (KSA-2 classifies documents the seller issues)
 * and the supply date (a seller obligation). Added: the supplier's own
 * invoice number and date, and an account column on every line — the debit
 * side of the bill, defaulted from the product and editable.
 *
 * Product picks copy the BUYING price. A null buying price leaves the field
 * at zero — it never falls through to the selling price, which is the
 * silent expense-overstatement trap this form was reviewed against.
 */
class PurchaseInvoiceForm
{
            ...
            Section::make(__('purchases.invoices.sections.details'))
                ->schema([
                    ...
                    TextInput::make('supplier_invoice_number')
                        ...
                        ->helperText(__('purchases.invoices.hints.supplier_invoice_number'))
                        ->maxLength(100)
                        // The friendly face of the composite unique index:
                        // the same paper bill keyed twice doubles expense,
                        // VAT and payables while balancing perfectly.
                        ->unique(
                            ignoreRecord: true,
                        ...
                        ->validationMessages([
                            'unique' => __('purchases.invoices.errors.duplicate_supplier_invoice_form'),
                        ]),

                    DatePicker::make('supplier_invoice_date')
                        ->label(__('purchases.invoices.fields.supplier_invoice_date'))
                        ->native(false),

                    DatePicker::make('issue_date')
                        ->label(__('purchases.invoices.fields.issue_date'))
                        ->helperText(__('purchases.invoices.hints.issue_date'))
                        ->native(false)
                        ->default(now())
                        ->required(),

                    DatePicker::make('due_date')
                        ->label(__('purchases.invoices.fields.due_date'))
                        ->native(false)
                        ->default(now())
                        ->required()
            ...
            Section::make(__('purchases.invoices.sections.notes'))
                ->schema([
                    Textarea::make('terms_and_conditions')
                        ->label(__('purchases.invoices.fields.terms_and_conditions')),
     ...
     *
     * The buying price and the expense account — never the selling price.
     * A product without a buying price leaves the field alone rather than
     * borrowing the sales figure.
     */
    ...
    /**
     * Products this company buys — the mirror of sellableProducts.
     *
